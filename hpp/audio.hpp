        ...
        while (rb.size() - consumed >= (size_t)(fs * ch)) {
            ...
            if (sr != opusRate) {
                ...
                for (int i = 0; i < opusFrameSamples * ch; i++) {
                    double srcIdx = (i / ch) * ratio * ch + (i % ch);
                    int idx0 = (int)srcIdx;
                    eb[i] = (int16_t)(std::clamp(src[idx0] * (1.f - (float)(srcIdx - idx0)) +
                        src[std::min(idx0 + ch, fs * ch - 1)] * (float)(srcIdx - idx0), -1.f, 1.f) * 32767.f);
                }
            } else {
            ...
            if (nb > 0) {
                ...
                if (pq.size() < 50) { pq.push({std::vector<uint8_t>(ob.begin(), ob.begin() + nb), ts, opusFrameSamples}); }
                qcv.notify_one();
            }
        }
...
public:
    AudioCapture() {
        ...
        if (FAILED(CoCreateInstance(__uuidof(MMDeviceEnumerator), 0, CLSCTX_ALL, __uuidof(IMMDeviceEnumerator), (void**)&en)) ||
            FAILED(en->GetDefaultAudioEndpoint(eRender, eConsole, &dv)) ||
            FAILED(dv->Activate(__uuidof(IAudioClient), CLSCTX_ALL, 0, (void**)&ac)) ||
            FAILED(ac->GetMixFormat(&wf)))
        ...
        if (FAILED(ac->Initialize(AUDCLNT_SHAREMODE_SHARED, AUDCLNT_STREAMFLAGS_LOOPBACK, 200000, 0, wf, 0)) ||
            FAILED(ac->GetService(__uuidof(IAudioCaptureClient), (void**)&cc)))
    ...
    void Start() {
        if (run) return;
        run = cap = true;
        if (FAILED(ac->Start())) { run = cap = false; return; }
    ...
    bool PopPacket(AudioPacket& o, int ms = 10) {
        ...
        if (!qcv.wait_for(lk, std::chrono::milliseconds(ms), [this] { return !pq.empty() || !run; }) || pq.empty()) return false;
