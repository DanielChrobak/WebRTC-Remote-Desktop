...
class AV1Encoder {
    ...
    bool InitHW(const AVCodec* c) {
        ...
        auto* d = (AVD3D11VADeviceContext*)((AVHWDeviceContext*)hd->data)->hwctx;
        d->device = dev; d->device_context = ctx;
        ...
        auto* fctx = (AVHWFramesContext*)hfr->data;
        fctx->format = AV_PIX_FMT_D3D11; fctx->sw_format = AV_PIX_FMT_BGRA;
        fctx->width = w; fctx->height = h; fctx->initial_pool_size = 4;
        if (av_hwframe_ctx_init(hfr) < 0) { av_buffer_unref(&hfr); av_buffer_unref(&hd); return false; }
        cc->hw_frames_ctx = av_buffer_ref(hfr); cc->pix_fmt = AV_PIX_FMT_D3D11;
        return true;
    }
...
public:
    AV1Encoder(int W, int H, int fps, ID3D11Device* d, ID3D11DeviceContext* c, ID3D11Multithread* m)
        ...
        for (auto n : {"av1_nvenc", "av1_qsv", "av1_amf", "libsvtav1", "libaom-av1"})
            if ((cd = avcodec_find_encoder_by_name(n))) { LOG("Encoder: %s", n); break; }
        ...
        cc->bit_rate = 20000000; cc->rc_max_rate = cc->rc_buffer_size = 40000000;
        cc->gop_size = fps * 2; cc->max_b_frames = 0;
    ...
    EncodedFrame* Encode(ID3D11Texture2D* tx, int64_t ts, bool fk = false) {
        ...
        o->ts = ts; o->encUs = ((t2.QuadPart - t1.QuadPart) * 1000000) / qf; o->isKey = gk;
        ec++; return o;
    }
