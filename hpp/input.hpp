...
inline WORD JsKeyToVK(uint16_t k) {
    ...
    static const std::unordered_map<uint16_t, WORD> m = {
        {8,VK_BACK},{9,VK_TAB},{13,VK_RETURN},{16,VK_SHIFT},{17,VK_CONTROL},{18,VK_MENU},{19,VK_PAUSE},{20,VK_CAPITAL},{27,VK_ESCAPE},{32,VK_SPACE},
        {33,VK_PRIOR},{34,VK_NEXT},{35,VK_END},{36,VK_HOME},{37,VK_LEFT},{38,VK_UP},{39,VK_RIGHT},{40,VK_DOWN},{44,VK_SNAPSHOT},{45,VK_INSERT},{46,VK_DELETE},
        {91,VK_LWIN},{92,VK_RWIN},{96,VK_NUMPAD0},{97,VK_NUMPAD1},{98,VK_NUMPAD2},{99,VK_NUMPAD3},{100,VK_NUMPAD4},{101,VK_NUMPAD5},{102,VK_NUMPAD6},
        {103,VK_NUMPAD7},{104,VK_NUMPAD8},{105,VK_NUMPAD9},{106,VK_MULTIPLY},{107,VK_ADD},{109,VK_SUBTRACT},{110,VK_DECIMAL},{111,VK_DIVIDE},
        {112,VK_F1},{113,VK_F2},{114,VK_F3},{115,VK_F4},{116,VK_F5},{117,VK_F6},{118,VK_F7},{119,VK_F8},{120,VK_F9},{121,VK_F10},{122,VK_F11},{123,VK_F12},
        {144,VK_NUMLOCK},{145,VK_SCROLL},{173,VK_VOLUME_MUTE},{174,VK_VOLUME_DOWN},{175,VK_VOLUME_UP},{176,VK_MEDIA_NEXT_TRACK},{177,VK_MEDIA_PREV_TRACK},
        {178,VK_MEDIA_STOP},{179,VK_MEDIA_PLAY_PAUSE},{186,VK_OEM_1},{187,VK_OEM_PLUS},{188,VK_OEM_COMMA},{189,VK_OEM_MINUS},{190,VK_OEM_PERIOD},
...
class InputHandler {
    ...
    void ToAbsolute(float nx, float ny, LONG& ax, LONG& ay) {
        int px = monX + (int)(std::clamp(nx, 0.f, 1.f) * monW), py = monY + (int)(std::clamp(ny, 0.f, 1.f) * monH);
    ...
    static bool IsExtendedKey(WORD vk) {
        return vk == VK_INSERT || vk == VK_DELETE || vk == VK_HOME || vk == VK_END || vk == VK_PRIOR || vk == VK_NEXT ||
               vk == VK_LEFT || vk == VK_RIGHT || vk == VK_UP || vk == VK_DOWN || vk == VK_LWIN || vk == VK_RWIN ||
...
public:
    ...
    void UpdateFromMonitorInfo(const MonitorInfo& mi) {
        ...
        if (GetMonitorInfo(mi.hMon, &info))
            SetMonitorBounds(info.rcMonitor.left, info.rcMonitor.top, info.rcMonitor.right - info.rcMonitor.left, info.rcMonitor.bottom - info.rcMonitor.top);
    }
    ...
    void WiggleCenter() {
        ...
        INPUT inp[3] = {}; DWORD fl = MOUSEEVENTF_MOVE | MOUSEEVENTF_ABSOLUTE | MOUSEEVENTF_VIRTUALDESK;
        for (int i = 0; i < 3; i++) { inp[i].type = INPUT_MOUSE; inp[i].mi.dwFlags = fl; inp[i].mi.dx = ax + (i == 1 ? 1 : 0); inp[i].mi.dy = ay; }
    ...
    void MouseMove(float nx, float ny) {
        ...
        INPUT inp{INPUT_MOUSE}; inp.mi.dwFlags = MOUSEEVENTF_MOVE | MOUSEEVENTF_ABSOLUTE | MOUSEEVENTF_VIRTUALDESK; inp.mi.dx = ax; inp.mi.dy = ay;
    ...
    void MouseWheel(int16_t dx, int16_t dy) {
        if (!enabled) return;
        if (dy) { INPUT inp{INPUT_MOUSE}; inp.mi.dwFlags = MOUSEEVENTF_WHEEL; inp.mi.mouseData = (DWORD)(-dy * WHEEL_DELTA / 100); SendInput(1, &inp, sizeof(INPUT)); }
        if (dx) { INPUT inp{INPUT_MOUSE}; inp.mi.dwFlags = MOUSEEVENTF_HWHEEL; inp.mi.mouseData = (DWORD)(dx * WHEEL_DELTA / 100); SendInput(1, &inp, sizeof(INPUT)); }
    }
    ...
    void Key(uint16_t jsKey, uint16_t scan, bool down, uint8_t) {
        ...
        INPUT inp{INPUT_KEYBOARD}; inp.ki.wVk = vk; inp.ki.wScan = scan ? scan : (WORD)MapVirtualKey(vk, MAPVK_VK_TO_VSC);
