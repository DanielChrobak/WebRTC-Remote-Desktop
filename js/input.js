...
const send = (type, ...a) => {
    ...
    let b;
    if (type === 'move') { b = mkBuf(12, v => { v.setUint32(0, MSG.MOUSE_MOVE, true); v.setFloat32(4, a[0], true); v.setFloat32(8, a[1], true); }); S.stats.moves++; }
    else if (type === 'btn') { b = mkBuf(6, v => { v.setUint32(0, MSG.MOUSE_BTN, true); v.setUint8(4, a[0]); v.setUint8(5, a[1] ? 1 : 0); }); S.stats.clicks++; }
    else if (type === 'wheel') { b = mkBuf(8, v => { v.setUint32(0, MSG.MOUSE_WHEEL, true); v.setInt16(4, Math.round(a[0]), true); v.setInt16(6, Math.round(a[1]), true); }); }
    else if (type === 'key') { b = mkBuf(10, v => { v.setUint32(0, MSG.KEY, true); v.setUint16(4, a[0], true); v.setUint16(6, a[1], true); v.setUint8(8, a[2] ? 1 : 0); v.setUint8(9, a[3]); }); S.stats.keys++; }
    else return;
    try { S.dc.send(b); } catch {}
};
...
const onDown = e => { if (S.controlEnabled || S.touchEnabled) { e.preventDefault(); send('btn', BMAP[e.button] ?? 0, true); } };
const onUp = e => { if (S.controlEnabled || S.touchEnabled) { e.preventDefault(); send('btn', BMAP[e.button] ?? 0, false); } };
const onWheel = e => { if (S.controlEnabled || S.touchEnabled) { e.preventDefault(); send('wheel', e.deltaX, e.deltaY); } };
const onCtx = e => S.controlEnabled && e.preventDefault();
const getMods = e => (e.ctrlKey ? 1 : 0) | (e.altKey ? 2 : 0) | (e.shiftKey ? 4 : 0) | (e.metaKey ? 8 : 0);
const onKeyD = e => { if (!S.controlEnabled) return; if (e.key === 'Escape' && S.pointerLocked) return exitLock(); !e.metaKey && e.preventDefault(); send('key', e.keyCode, 0, true, getMods(e)); };
const onKeyU = e => { if (!S.controlEnabled) return; !e.metaKey && e.preventDefault(); send('key', e.keyCode, 0, false, getMods(e)); };
...
export const enableControl = () => {
    ...
    canvas.addEventListener('mouseup', onUp); canvas.addEventListener('contextmenu', onCtx);
...
export const disableControl = () => {
    ...
    canvas.removeEventListener('mouseup', onUp); canvas.removeEventListener('contextmenu', onCtx);
...
const onTCancel = () => { clearLp(); tDrag && (send('btn', 0, false), console.info('Touch cancelled')); tId = null; tDrag = lpActive = pinch = false; };

const on2FStart = e => S.touchEnabled && e.touches.length === 2 && (tf2Start = performance.now());
const on2FEnd = e => {
...
export const enableTouch = () => {
    ...
    add('touchstart', onTStart, { passive: false }); add('touchmove', onTMove, { passive: false });
    add('touchend', onTEnd, { passive: false }); add('touchcancel', onTCancel);
    add('touchstart', on2FStart); add('touchend', on2FEnd);
    add('touchstart', onPStart, { passive: false }); add('touchmove', onPMove, { passive: false });
    add('touchend', onPEnd, { passive: false }); add('touchcancel', onPEnd);
    updateTouchUI();
};
