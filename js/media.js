...
export const initDecoder = async (force = false) => {
    ...
    S.ready = false; S.needKey = true; S.lastCapTs = 0; if (force) S.W = S.H = 0;
    ...
    for (const [hw, label] of [[true, 'HW'], [false, 'SW']]) {
        ...
        try { const sup = await VideoDecoder.isConfigSupported(cfg); if (sup.supported) { dec.configure(sup.config); S.hwAccel = label; console.info(`Decoder: ${label}`); break; } } catch {}
    }
    if (dec.state !== 'configured') { console.error('Decoder config failed'); S.hwAccel = 'NONE'; return; }
    S.ready = true; reqKey?.();
};
...
export const initAudio = async () => {
    ...
    try {
        ...
        S.audioCtx = ctx; S.audioGain = gain; if (ctx.state === 'suspended') await ctx.resume();
        if (window.AudioDecoder) {
            try { const d = new AudioDecoder({ output: ad => S.audioEnabled && S.audioCtx && playAudio(ad), error: e => console.warn('Audio decode:', e.message) });
                d.configure({ codec: 'opus', sampleRate: C.AUDIO_RATE, numberOfChannels: C.AUDIO_CH }); S.audioDecoder = d; } catch (e) { console.warn('AudioDecoder unavailable:', e.message); } }
        console.info('Audio initialized:', ctx.sampleRate, 'Hz'); return true;
    } catch (e) { console.error('Audio init failed:', e.message); return false; }
};

const playAudio = ad => {
    const { audioCtx: ctx, audioGain: gain } = S; if (!ctx || !gain) return;
    try {
        ...
        const dur = nf / sr, now = ctx.currentTime, needsReset = !S.audioPlaying || S.audioNextTime < now - 0.1 || S.audioNextTime > now + 0.5;
        if (needsReset && sources.length) { sources.forEach(s => { try { s.stop(); s.disconnect(); } catch {} }); sources = []; }
        ...
        sources.push(src); src.onended = () => { const i = sources.indexOf(src); i >= 0 && sources.splice(i, 1); };
        while (sources.length > 10) { try { sources.shift().disconnect(); } catch {} }
...
export const toggleAudio = () => {
    const btn = $('aBtn'), st = $('aSt'), stT = $('aStT'), btnT = $('aTxt');
    if (!S.audioEnabled) {
        initAudio().then(ok => { if (ok) { S.audioEnabled = S.audioPlaying = true; S.audioNextTime = 0; btn.classList.add('on'); st.classList.add('on'); btnT.textContent = 'Disable'; stT.textContent = 'Audio active'; } else stT.textContent = 'Audio failed'; });
