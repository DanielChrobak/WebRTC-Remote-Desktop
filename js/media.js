...
export const initDecoder = async (force = false) => {
    ...
    const decoder = new VideoDecoder({
        ...
        error: e => {
            ...
            if (!S.reinit) { S.reinit = true; setTimeout(() => { S.reinit = false; initDecoder(); }, 50); }
        }
    });
    S.decoder = decoder;
    ...
    for (const [hw, label] of [[true, 'HW'], [false, 'SW']]) {
        ...
        try {
            ...
            if (sup.supported) { decoder.configure(sup.config); S.hwAccel = label; console.info(`Decoder: ${label}`); break; }
        } catch {}
    }
...
export const decodeFrame = data => {
    ...
    try {
        if (S.decoder.decodeQueueSize > 8 && !data.isKey) { reqKey?.(); S.stats.tDropDec++; return; }
        ...
        if (S.frameMeta.size > 30) S.frameMeta.clear();
...
export const initAudio = async () => {
    ...
    try {
        ...
        const ctx = new AC({ sampleRate: C.AUDIO_RATE, latencyHint: 'interactive' });
...
const playAudio = ad => {
    ...
    try {
        ...
        const isPlanar = fmt.includes('planar'), isS16 = fmt.includes('s16');
        ...
        const st = !S.audioPlaying || S.audioNextTime < now || S.audioNextTime > now + 0.5 ? (S.audioPlaying = true, now + C.AUDIO_BUF) : S.audioNextTime;
...
export const handleAudioPkt = data => {
    ...
    if (len > data.byteLength - C.AUDIO_HEADER) return;
    if (S.audioDecoder?.state === 'configured') {
        try {
            S.audioDecoder.decode(new EncodedAudioChunk({ type: 'key', timestamp: performance.now() * 1000, duration: (samples / C.AUDIO_RATE) * 1e6, data: new Uint8Array(data, C.AUDIO_HEADER, len) }));
        } catch {}
    }
};

export const toggleAudio = () => {
    const btn = document.getElementById('aBtn'), st = document.getElementById('aSt');
    const stT = document.getElementById('aStT'), btnT = document.getElementById('aTxt');

    if (!S.audioEnabled) {
        initAudio().then(ok => {
            if (ok) {
                S.audioEnabled = S.audioPlaying = true; S.audioNextTime = 0;
                btn.classList.add('on'); st.classList.add('on');
                btnT.textContent = 'Disable Audio'; stT.textContent = 'Audio streaming active';
            } else stT.textContent = 'Audio initialization failed';
        });
    } else {
        S.audioEnabled = S.audioPlaying = false; S.audioNextTime = 0;
        btn.classList.remove('on'); st.classList.remove('on');
        btnT.textContent = 'Enable Audio'; stT.textContent = 'Click to enable audio playback';
    }
};
