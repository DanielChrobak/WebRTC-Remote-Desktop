...
export const initDecoder = async (force = false) => {
    ...
    for (const [hw, label] of [[true, 'HW'], [false, 'SW']]) {
        ...
        try {
            ...
            if (sup.supported) { dec.configure(sup.config); S.hwAccel = label; console.info(`Decoder: ${label}`); break; }
        } catch {}
    }
...
export const decodeFrame = data => {
    if (!S.ready || (!data.isKey && S.needKey)) { S.stats.tDropDec++; if (!S.ready) reqKey?.(); return; }
...
export const initAudio = async () => {
    ...
    try {
        ...
        if (window.AudioDecoder) {
            try {
                const d = new AudioDecoder({ output: ad => S.audioEnabled && S.audioCtx && playAudio(ad), error: e => console.warn('Audio decode:', e.message) });
                d.configure({ codec: 'opus', sampleRate: C.AUDIO_RATE, numberOfChannels: C.AUDIO_CH });
                S.audioDecoder = d;
...
const playAudio = ad => {
    ...
    try {
        ...
        for (let ch = 0; ch < nc; ch++) {
            ...
            const tv = isS16 ? new Int16Array(ad.allocationSize({ planeIndex: pi }) / 2) : new Float32Array(ad.allocationSize({ planeIndex: pi }) / 4);
...
export const handleAudioPkt = data => {
    ...
    const samples = v.getUint16(12, true), len = v.getUint16(14, true);
