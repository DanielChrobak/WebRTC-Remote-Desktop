/**
 * @file media.js
 * @brief Video and audio decoding functionality
...
/**
 * Sets the keyframe request callback.
...
/**
 * Initializes or reinitializes the video decoder.
 * @param {boolean} force - Force full reinitialization
 */
export const initDecoder = async (force = false) => {
    ...
    for (const [hw, label] of [[true, 'HW'], [false, 'SW']]) {
        ...
        try {
            ...
            if (sup.supported) {
                dec.configure(sup.config);
                S.hwAccel = label;
                console.info(`Decoder: ${label}`);
                break;
            }
        } catch {}
    }
    ...
    if (dec.state !== 'configured') {
        console.error('Decoder config failed');
        S.hwAccel = 'NONE';
        return;
    }
 ...
 * @param {Object} data - Frame data object
 */
export const decodeFrame = data => {
    if (!S.ready || (!data.isKey && S.needKey)) {
        S.stats.tDropDec++;
        if (!S.ready) reqKey?.();
        return;
    }
    ...
    try {
        ...
        if (S.frameMeta.size > 30) {
            [...S.frameMeta.keys()]
                .sort((a, b) => a - b)
                .slice(0, -20)
                .forEach(k => S.frameMeta.delete(k));
        }

        const dur = S.lastCapTs > 0 && data.capTs > S.lastCapTs
            ? data.capTs - S.lastCapTs
            : 16667;
...
/**
 * Initializes the audio context and decoder.
        ...
        if (window.AudioDecoder) {
            try {
                const d = new AudioDecoder({
                    output: ad => S.audioEnabled && S.audioCtx && playAudio(ad),
                    error: e => console.warn('Audio decode:', e.message)
                });
                ...
                console.warn('AudioDecoder unavailable:', e.message);
            }
        }
...
/**
 * Plays decoded audio data.
 * @param {AudioData} ad - Decoded audio data
 */
const playAudio = ad => {
    ...
    try {
        ...
        const buf = ctx.createBuffer(nc, nf, sr);
        const isPlanar = fmt.includes('planar');
        const isS16 = fmt.includes('s16');

        for (let ch = 0; ch < nc; ch++) {
            const pi = isPlanar ? ch : 0;
            const sz = ad.allocationSize({ planeIndex: pi });
            const tv = isS16 ? new Int16Array(sz / 2) : new Float32Array(sz / 4);
            const cd = new Float32Array(nf);

            ad.copyTo(tv, { planeIndex: pi });

            for (let i = 0; i < nf; i++) {
                cd[i] = isS16
                    ? tv[isPlanar ? i : i * nc + ch] / 32768
                    : tv[isPlanar ? i : i * nc + ch];
            }
        ...
        const needsReset = !S.audioPlaying ||
                          S.audioNextTime < now - 0.1 ||
                          S.audioNextTime > now + 0.5;

        if (needsReset && sources.length) {
            sources.forEach(s => {
                try {
                    s.stop();
                    s.disconnect();
        ...
        const st = needsReset
            ? (S.audioPlaying = true, now + C.AUDIO_BUF)
            : S.audioNextTime;
 ...
 * @param {ArrayBuffer} data - Audio packet data
 */
export const handleAudioPkt = data => {
    ...
    const samples = v.getUint16(12, true);
    const len = v.getUint16(14, true);
    ...
    try {
        S.audioDecoder.decode(new EncodedAudioChunk({
            ...
            duration: (samples / C.AUDIO_RATE) * 1e6,
...
export const toggleAudio = () => {
    ...
    if (!S.audioEnabled) {
        initAudio().then(ok => {
            if (ok) {
                S.audioEnabled = S.audioPlaying = true;
                S.audioNextTime = 0;
                btn.classList.add('on');
                st.classList.add('on');
                btnT.textContent = 'Disable';
                stT.textContent = 'Audio active';
            } else {
                stT.textContent = 'Audio failed';
            }
        });
    } else {
        S.audioEnabled = S.audioPlaying = false;
        S.audioNextTime = 0;
        btn.classList.remove('on');
        st.classList.remove('on');
        btnT.textContent = 'Enable';
        stT.textContent = 'Tap to enable audio';
    }
};
