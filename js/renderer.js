...
const initWebGL = () => {
    ...
    if (!gl.getProgramParameter(pg, gl.LINK_STATUS)) return console.error('Link error:', gl.getProgramInfoLog(pg)), false;
...
export const renderZoomed = () => {
    ...
    updateSize();
    clearAndDraw(calcVp(S.W, S.H, canvasW, canvasH));
};
...
const handleResize = () => {
    updateSize();
    if (S.W > 0 && S.H > 0 && gl) clearAndDraw(calcVp(S.W, S.H, canvasW, canvasH));
};

let rto;
window.addEventListener('resize', () => { clearTimeout(rto); rto = setTimeout(handleResize, 50); });
window.addEventListener('orientationchange', () => { setTimeout(handleResize, 100); setTimeout(handleResize, 300); });
...
window.visualViewport?.addEventListener('resize', () => { clearTimeout(rto); rto = setTimeout(handleResize, 50); });
