...
const msgs = {
    [Stage.IDLE]: ['Connecting...', 'Initializing'], [Stage.ICE]: ['Connecting...', 'Gathering ICE'],
    [Stage.SIGNAL]: ['Connecting...', 'Signaling'], [Stage.CONNECT]: ['Connecting...', 'Peer connection'],
    [Stage.AUTH]: ['Authenticating...', 'Verifying'], [Stage.STREAM]: ['Almost there...', 'Waiting for stream'],
...
export const updateLoadingStage = (stage, errMsg = null) => {
    ...
    stageMap.forEach(([el, s]) => {
        ...
        const c = stage === Stage.ERR ? (i <= idx ? 'error' : '') : i < idx ? 'done' : i === idx ? 'active' : '';
...
const fmtArgs = a => {
    ...
    for (let i = 0; i < a.length; i++) {
        const x = a[i];
        if (x == null) { r.push(String(x)); continue; }
        if (typeof x === 'string') {
            const s = x.replace(/%c/g, '');
            while (i + 1 < a.length && typeof a[i + 1] === 'string' && /color:|font|background/.test(a[i + 1])) i++;
            if (s.trim()) r.push(s);
        } else r.push(typeof x === 'object' ? JSON.stringify(x) : String(x));
    }
    return r.join(' ');
};
...
const togglePnl = on => ['pnl', 'bk', 'edge'].forEach(id => $(id).classList.toggle('on', on));
...
document.querySelectorAll('input[name="tm"]').forEach(r => r.addEventListener('change', e => { if (e.target.checked) setTouchMode(e.target.value); }));
...
if (!(document.fullscreenEnabled || document.webkitFullscreenEnabled || document.documentElement.requestFullscreen || document.documentElement.webkitRequestFullscreen)) {
...
$('fs').onclick = () => {
    ...
    else (document.documentElement.requestFullscreen || document.documentElement.webkitRequestFullscreen)?.call(document.documentElement).catch(e => console.warn('Fullscreen:', e.message));
};
...
const handleFullscreenChange = async () => {
    ...
    if (supportsKeyboardLock) {
        if (document.fullscreenElement || document.webkitFullscreenElement) {
            try { await navigator.keyboard.lock(['Escape']); keyboardLocked = true; console.info('Keyboard locked - hold Escape to exit fullscreen'); }
            catch (e) { console.warn('Keyboard lock failed:', e.message); keyboardLocked = false; }
        } else { navigator.keyboard.unlock(); keyboardLocked = false; console.info('Keyboard unlocked'); }
    }
};

['fullscreenchange', 'webkitfullscreenchange'].forEach(e => document.addEventListener(e, handleFullscreenChange));

export const exitFullscreen = () => { if (isFs()) (document.exitFullscreen || document.webkitExitFullscreen).call(document); };
...
export const updateStats = () => {
    ...
    if (!S.statsOn) { resetStats(); return; }
